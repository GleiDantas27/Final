...
function Luhn(card) {
    "use strict";
    var cc = card, i, k, len, sNumber, total = 0, aux = 0, aux1 = 0, val = 0, val1 = 0, array = [];
...
function checkCreditCard(card) {
    "use strict";
    var cc = card.value, ccfd = cc.charAt(0), ccdd = cc.substr(0, 2), cclen = cc.length, luhn;
...
var setfocus = function (textReq) {
    "use strict";
    var x = textReq.value, el = textReq;
    if (x.length === 0) {
...
function checkExpiration() {
    "use strict";
    var actualDate = new Date(), selectedMonth = $("monthB").selectedIndex, selectedYear = $("yearB").options[$("yearB").selectedIndex].value, actualMonth = actualDate.getMonth(), actualYear = actualDate.getFullYear();
...
function changecat(value) {
    ...
    if (value.length === 0) {
        ...
        for (sizeId in sizesByDough[value]) {
            if (sizesByDough[value].hasOwnProperty(sizeId)) {
...
function validateInfo() {
    "use strict";
    var i, flag = true, nodes = document.querySelectorAll("#delivery input[type=text]"), el, displayValue, x, ind;
    ...
    ind = $("sizes").selectedIndex;
    if (ind === 0) {
        addClass($("sizes"), "invalid");
    } else {
        removeClass($("sizes"), "invalid");
    }
    x = document.getElementsByClassName("invalid");
    if (x.length) {
        flag = false;
    }
    return flag;
...
function validateInfoB() {
    "use strict";
    var i, flag = true, nodes = document.querySelectorAll("#billingf input[type=text]"), ind, x;
    for (i = 0; i < nodes.length; i += 1) {
        if (nodes[i].id !== "aptB") {
            inputRequired($(nodes[i].id));
        }
    }
    ind = $("monthB").selectedIndex;
    if (ind === 0) {
        addClass($("monthB"), "invalid");
    } else {
        removeClass($("monthB"), "invalid");
...
$("sizes").addEventListener("change", function () {
    "use strict";
    var e = $("sizes"), val = e.options[e.selectedIndex].value;
...
$("cheese").addEventListener("change", function () {
    "use strict";
    var e = $("cheese"), val = e.options[e.selectedIndex].value;
...
$("sauce").addEventListener("change", function () {
    "use strict";
    var e = $("sauce"), val = e.options[e.selectedIndex].value;
...
window.addEventListener("load", function () {
    ...
    var radioOption = [document.getElementsByName('dough')[0],  document.getElementsByName('dough')[1], document.getElementsByName('dough')[2], document.getElementsByName('dough')[3]], el = $('toppings'), tops = el.getElementsByTagName('input'), i, initialValue;
    ...
    $('orderPizza').addEventListener("click", function () {
        if (validateInfo() === true) {
            var el = $("billing"), displayValue = el.style.display;
            if (displayValue === "block") {
